...
const createTransporter = () => {
  const emailUser = readEnvValue("EMAIL_USER", "email_user");
  const emailPassword = readEnvValue("EMAIL_PASSWORD", "email_password");
  const emailFrom = readEnvValue("EMAIL_FROM", "email_from") || emailUser;
  const smtpHost = readEnvValue("SMTP_HOST", "smtp_host") || "smtp.gmail.com";
  const smtpPort = Number(readEnvValue("SMTP_PORT", "smtp_port") || 587);
  const smtpSecure = readEnvValue("SMTP_SECURE", "smtp_secure") === "true" || smtpPort === 465;
...
const sendVerificationEmail = async (email, verificationToken, username, verificationUrl) => {
  ...
  if (!emailUser || !emailPassword || !emailFrom) {
    const message = "Thiếu cấu hình Gmail. Vui lòng thiết lập EMAIL_USER (hoặc email_user), EMAIL_PASSWORD (hoặc email_password) và EMAIL_FROM.";
